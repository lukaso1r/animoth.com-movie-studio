...
const VideoItemMobile: React.FC<any> = ({ video }) => {
  ...
  return (
    ...
    <Link 
        ...
        >
            <img src={video.thumbnail.formats.small.url} alt={video.title} className="w-full h-full object-cover" />
            
            <div className='absolute bottom-0 left-4 w-full'>
                <h3 className="text-white text-2xl font-semibold mb-2 text-shadow-lg">{video?.title}</h3>
            </div> 
